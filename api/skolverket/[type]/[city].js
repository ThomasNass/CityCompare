...
export default async function handler(req, res) {
  ...
  const endpoint = SKOLVERKET_ENDPOINTS[type];
  if (!endpoint || !city) {
  ...
  try {
    ...
    if (!isScbTable(data)) {
      return res.status(status >= 400 ? status : 502).json(data ?? { error: "Skolverket-anropet misslyckades" });
    }
    return res.status(status).json(data);
  } catch (error) {
    return res.status(502).json({ error: error.message || "Skolverket-anropet misslyckades" });
  }
}
